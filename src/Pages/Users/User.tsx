...
const User = () => {
  ...
  const [vehiculoRegistrado, setVehiculoRegistrado] = useState<any | null>(null);
  ...
  useEffect(() => {
    const storedVehiculo = JSON.parse(localStorage.getItem('vehiculoRegistrado') || 'null');
    setVehiculoRegistrado(storedVehiculo);
  }, []);
  ...
  const handleReplaceVehiclePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file && vehiculoRegistrado) {
      const reader = new FileReader();
      reader.onloadend = () => {
        const updatedVehicle = { ...vehiculoRegistrado, fotoDelantera: reader.result as string };
        setVehiculoRegistrado(updatedVehicle);
        localStorage.setItem('vehiculoRegistrado', JSON.stringify(updatedVehicle));
      };
      reader.readAsDataURL(file);
    }
  };
        ...
        <div className="user__form-right">
          ...
          <div className="user__input-group user__input-group--vehicles">
            ...
            <div className="user__vehicle-photo-list">
              {vehiculoRegistrado && (
                <div className="user__vehicle-photo-item">
                  <span className="user__vehicle-photo-name">
                    {vehiculoRegistrado.tipo} - {vehiculoRegistrado.placa}
                  </span>
